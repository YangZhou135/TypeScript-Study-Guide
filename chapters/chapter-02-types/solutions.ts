...
function handleEvent(event: UIEvent): void {
    switch (event.type) {
        case 'click':
            console.log(`点击 ${event.target} 在坐标 (${event.coordinates.x}, ${event.coordinates.y})`);
            break;
        case 'keypress':
            console.log(`按键 ${event.key}`, event.ctrlKey ? '(Ctrl)' : '', event.shiftKey ? '(Shift)' : '');
            break;
        case 'scroll':
...
// 7. 定义文章操作类型
type ArticleAction = 
    | { type: 'create'; article: Omit<Article, 'id' | 'createdAt' | 'updatedAt' | 'viewCount' | 'likeCount' | 'comments'> }
    | { type: 'update'; id: string; updates: Partial<Pick<Article, 'title' | 'content' | 'category' | 'tags'>> }
