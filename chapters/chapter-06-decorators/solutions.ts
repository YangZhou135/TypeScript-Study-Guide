        ...
        descriptor.value = async function (...args: any[]) {
            return Promise.race([
                ...
                new Promise((_, reject) => {
                    setTimeout(
                        () => reject(new Error(`方法 ${propertyKey} 执行超时 (${ms}ms)`)),
                        ms
                    );
                }),
            ]);
    ...
    return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = function (...args: any[]) {
            ...
            if (!roles.includes(currentUserRole)) {
                throw new Error(
                    `权限不足: 需要角色 [${roles.join(", ")}]，当前角色: ${currentUserRole}`
                );
            }
...
class OrderService {
    ...
    @validateParams
    updateOrderStatus(@notNull @min(1) orderId: number, @notNull status: string): void {
