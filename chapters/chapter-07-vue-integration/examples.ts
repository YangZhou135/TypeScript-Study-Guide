...
const userModule = {
    ...
    actions: {
        ...
        async login({ commit }: any, credentials: { email: string; password: string }): Promise<void> {
            commit('SET_LOADING', true);
...
const ApiPlugin: VuePlugin = {
    install(Vue: any, options: { baseURL: string }) {
        ...
        // 添加全局方法
        Vue.prototype.$showMessage = function(message: string, type: 'success' | 'error' = 'success') {
